...
# part 1
document <- readLines('day-16.txt')

fields <- document[1:20]
...
otherTicketValues <- t(sapply(document[26:262], function(x){as.integer(unlist(strsplit(x, split = ',')))}))
rownames(otherTicketValues) <- NULL
sum(unlist(otherTicketValues)[which(!(unlist(otherTicketValues) %in% fullRanges))])
...
myTicket <- as.integer(unlist(strsplit(document[23], split = ',')))
as.character(prod(myTicket[c(
